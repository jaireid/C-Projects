...
/*
After function has finished executing, n is overwritten with the number of distinct elements of arr, and the first n elements (using the updated n-value) of arr are overwritten with the distinct elements in ascending order. Also, the first n elements (using the updated n-value) of freq are overwritten with the frequencies of the corresponding elements in arr.
*/
...
// Merge the distinct elements and frequencies together.
  ...
  // If the element at index i in the left subarray is smaller than the element at index j in the right subarray then increment i
		else if (arr[i] < arr[j]) {
			i++;
		}
  // If elements are equal add frequencies
		else {
			freq[i] += freq[j];
			merges++;
      // If the element's frequency is greater than or equal to k output the element
